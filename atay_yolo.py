...
import cv2
import numpy as np
from atay_zed import get_object_z
import tensorrt as trt
import pycuda.driver as cuda
import atay_yolo_classes
...
def YOLO_Visulation(zed,detection,cls_dict):
    """
    detection[0] = box
    detection[1] = conf
    detection[2] = clss
    zed.image    = image
    zed.point_cloud = point cloud
    """
    image = zed.image
    point_cloud = zed.point_cloud
    detect = []
    overlay = image.copy()
    for bb, cf, cl in zip(detection[0], detection[1], detection[2]):
        cl = int(cl)
        cls_name = cls_dict.get(cl, 'CLS{}'.format(cl))
        x_min, y_min, x_max, y_max = bb[0], bb[1], bb[2], bb[3]
        z = get_object_z(point_cloud,[x_min, y_min, x_max, y_max],area_div=3)
        #cv2.rectangle(image_ocv, (x_min, y_min), (x_max, y_max), (0,0,255), 2)
        cv2.rectangle(overlay, (x_min, y_min), (x_max, y_max), (0,0,255), -1)
        cv2.putText(overlay,cls_name + " " + str("{:.2f}".format(z))+"m",(x_min, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, [0, 0, 255], 5)
        #cv2.putText(image_ocv,cls_name + " " + str("{:.2f}".format(z))+"m",(x_min, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0, 0, 255], 2)
        detect.append([cls_name,cf,z])
...
class TrtYOLOv3(object):
    ...
    def __init__(self, model, input_shape=(416, 416), category_num=17):
